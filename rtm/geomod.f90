! ---------------------------------------------------------------------------
!
! The functions in this module comprise a radiative transfer model to
! determine the top-of-atmosphere TBs based on various inputs.
!
! References:
!
...
module geomod
  ...
  implicit none
  private
  public :: fdem0_meissner_wentz, dielectric_meissner_wentz

  integer, parameter :: NSTOKE=4

contains

  subroutine fdem0_meissner_wentz(freq,tht,sst, em0)
    real(real32), intent(in) :: freq,tht,sst
    real(real32), dimension(2), intent(out) :: em0

    real(real32), parameter :: f0=17.97510
    real(real32), parameter :: salinity=35.

    real(real32), save :: sstsv = 1.e30, thtsv = 1.e30
    real(real32), save :: costht,sinsqtht
    real(real32), save :: e0s,e1s,e2s,n1s,n2s,sig

    complex(real32) :: permit,esqrt,rh,rv
    complex(real32), parameter :: j = (0., 1.)

    if(abs(sst-sstsv).gt.0.001) then
       sstsv=sst
       call dielectric_meissner_wentz(sst,salinity,  e0s,e1s,e2s,n1s,n2s,sig)
    endif

    if(abs(tht-thtsv).gt.0.001) then
       thtsv=tht
       costht=cosd(tht)
    ...
    em0(1)=1.-real(rv*conjg(rv), real32)
    em0(2)=1.-real(rh*conjg(rh), real32)
